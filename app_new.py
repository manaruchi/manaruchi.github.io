...
with open(os.path.join(cur_folder, '100m_corr.geojson')) as f:
    pipe_json1 = json.load(f)
with open(os.path.join(cur_folder, '150m_corr.geojson')) as f:
    pipe_json2 = json.load(f)
with open(os.path.join(cur_folder, '200m_corr.geojson')) as f:
    pipe_json3 = json.load(f)
...
def select_ground_water16(n1,n2,n3,n4,n5,n6,n7):
    ...
    if(n7):
        return [{"label": "Overview", "value": "overview"},
                ...
                {"label": "Pipe Distribution", "value": "ppdis"},
                ]
...
def select_ground_water18(selected_val, zoneshow, op, bmap):
    ...
    if(ncc == [0,0,0,0,0,0,0,1]):
        ...
        if(selected_val == 'overview'):
        ...
        elif(selected_val == 'ppdis'):

            pipe_data = []

            for feature in pipe_json1["features"]:
                pipe_data.append(go.Scattermapbox(
                        lat=np.array(feature["geometry"]["coordinates"])[0][:,1],
                        lon=np.array(feature["geometry"]["coordinates"])[0][:,0],
                        mode="lines",
                        hoverinfo = 'none',
                        showlegend=False,
            ...
            for feature in pipe_json2["features"]:
                pipe_data.append(go.Scattermapbox(
                        lat=np.array(feature["geometry"]["coordinates"])[0][:,1],
                        lon=np.array(feature["geometry"]["coordinates"])[0][:,0],
                        mode="lines",
                        hoverinfo = 'none',
                        showlegend=False,
                        line=dict(width=8, color="#00FF00")
                    ))

            for feature in pipe_json3["features"]:
                pipe_data.append(go.Scattermapbox(
                        lat=np.array(feature["geometry"]["coordinates"])[0][:,1],
                        lon=np.array(feature["geometry"]["coordinates"])[0][:,0],
                        mode="lines",
                        hoverinfo = 'none',
                        showlegend=False,
                        line=dict(width=8, color="#FF00FF")
                    ))



            fig = go.Figure(data=pipe_data)

            fig.update_layout(mapbox_style=bmap,
                              mapbox_zoom=15, mapbox_center = {"lat": 30.3273 , "lon": 77.995})
    ...
    if(zonebut):   #Zone
        ...
        if(selected_val == 'overview'):
            ...
            return html.Div([
                ...
                html.P('Distribution Network', style = {'font-weight': 'bold', 'text-align': 'center'}),



                html.Table([html.Tr([html.Th('Color'), html.Th('Description')]),
                           html.Tr([html.Td(style = {'background-color': '#F00'}), html.Td('100 m', style = {'padding-left': '2px'})]),
                           html.Tr([html.Td(style = {'background-color': '#00FF00'}), html.Td('150 m', style = {'padding-left': '2px'})]),

                           html.Tr([html.Td(style = {'background-color': '#FF00FF'}), html.Td('200 m', style = {'padding-left': '2px'})]),],style = {'width': '90%', 'font-size': 'small'})
            ])
